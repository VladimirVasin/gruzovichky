...
public partial class GameBootstrap : MonoBehaviour
{
    ...
    private static int GetWorkerLeisureChoiceWeight(WorkerWeaknessKind weakness, LocationType type)
    {
        int weight = weakness switch
        {
            ...
            _ => type switch
            {
                LocationType.CityPark => 5,
                LocationType.GamblingHall => 3,
                LocationType.Bar => 3,
                _ => 1
            }
        };

        return Mathf.Clamp(weight, 1, 8);
    }
    ...
    private bool CompleteWorkerVendorPurchase(DriverAgent driver, DriverRescuePhase completedPhase, Vector3 purchasePosition)
    {
        ...
        ApplyServiceSaleTaxes(driver, vendor, price, $"{GetWorkerInventoryItemTitle(itemId, false)} purchase");
