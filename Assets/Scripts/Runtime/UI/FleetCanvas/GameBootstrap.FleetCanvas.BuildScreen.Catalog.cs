    ...
    private string GetBuildDescription(BuildTool tool, bool isActive)
    {
        ...
        if (isActive)
        {
            return tool switch
            {
                ...
                BuildTool.Kiosk            => ru ? $"Режим активен: поставь киоск 2x1 без подъезда к дороге. Snack и Coffee покупаются здесь за $5. R — поворот ({rot})." : $"Mode active: place a 2x1 walk-up kiosk. Snacks and Coffee cost $5 here. No road driveway required. R rotates ({rot}).",
        ...
        return tool switch
        {
            ...
            BuildTool.Kiosk            => ru ? "Киоск: рабочие подходят к стойке и покупают Snack или Coffee за $5 в инвентарь. Подъезд к дороге не нужен." : "Walk-up kiosk: workers buy a $5 Snack or Coffee for their inventory. No road driveway required.",
