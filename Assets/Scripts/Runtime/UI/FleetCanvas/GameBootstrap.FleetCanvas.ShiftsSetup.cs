...
public partial class GameBootstrap
{
    private void SetupShiftsScreenUi()
    {
        ...
        LocationType[] serviceTypes = { LocationType.Motel, LocationType.Bar, LocationType.Canteen, LocationType.GasStation, LocationType.GamblingHall, LocationType.Kindergarten, LocationType.CarMarket, LocationType.LaborExchange, LocationType.CleaningDepot };
        for (int si = 0; si < serviceTypes.Length; si++)
        {
            int maxWorkerSlots = GetMaxBuildingWorkerSlots(serviceTypes[si]);
            for (int workerSlot = 0; workerSlot < maxWorkerSlots; workerSlot++)
            {
                slotArrayIndex = AddBuildingWorkerSlotCard(logisticsContent, font, slotArrayIndex, serviceTypes[si], workerSlot);
            }
        }
