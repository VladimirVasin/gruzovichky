...
- One partial `MonoBehaviour` (`GameBootstrap`) constructs and runs nearly the full prototype:
  ...
  - worker needs/personality/life-cycle/knowledge logic
...
- Numeric worker skills remain removed; worker personality is split into character traits, at most one weakness, and temporary affects. `GameBootstrap.WorkerPerks*` still owns trait/weakness assignment and Workers UI, `GameBootstrap.WorkerAffects.cs` owns affect lifecycle/thought/knowledge hooks, and `GameBootstrap.WorkerEducation.cs` keeps the small Basic/Vocational/Higher gate needed by Labor Exchange staffing.
