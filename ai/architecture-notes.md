...
- Numeric worker skills remain removed; worker personality is split into traits, one leisure preference, and temporary affects. `GameBootstrap.WorkerPerks*` still owns trait/leisure assignment and Workers UI, `GameBootstrap.WorkerAffects.cs` owns affect lifecycle/thought/knowledge hooks, and `GameBootstrap.WorkerEducation.cs` keeps the small Basic/Vocational/Higher gate needed by Labor Exchange staffing.
